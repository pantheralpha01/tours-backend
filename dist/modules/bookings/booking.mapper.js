...
exports.mapUpdateDTOToDb = exports.mapCreateDTOToDb = void 0;
const client_1 = require("@prisma/client");
/**
 * Calculate commission based on amount and rate
 */
const calculateCommission = (amount, rate = 0.15) => {
    return parseFloat((amount * rate).toFixed(2));
};
/**
 * Convert USD to KES (approximate rate)
 */
const convertUsdToKes = (usdAmount, rate = 150) => {
...
const mapCreateDTOToDb = (dto) => {
    ...
    const commissionRate = new client_1.Prisma.Decimal(0.15); // 15%
    const commissionAmount = new client_1.Prisma.Decimal(calculateCommission(dto.amount || 0, 0.15));
    // Convert commission to KES if needed
    const commissionInKes = new client_1.Prisma.Decimal(convertUsdToKes(commissionAmount.toNumber(), 150));
    ...
    if (dto.depositAmount !== undefined && dto.depositAmount !== null) {
        ...
        balanceAmount = new client_1.Prisma.Decimal((dto.totalAmount || dto.amount || 0) - (dto.depositAmount || 0));
    }
    return {
        ...
        costAtBooking: dto.costAtBooking
            ? new client_1.Prisma.Decimal(dto.costAtBooking)
            : null,
        costPostEvent: dto.costPostEvent
            ? new client_1.Prisma.Decimal(dto.costPostEvent)
            : null,
        totalCost: dto.totalCost ? new client_1.Prisma.Decimal(dto.totalCost) : null,
        depositPercentage: dto.depositPercentage
            ? new client_1.Prisma.Decimal(dto.depositPercentage)
            : null,
        depositAmount,
        balanceAmount,
...
const mapUpdateDTOToDb = (dto) => {
    ...
    if (dto.amount !== undefined) {
        updateData.amount = new client_1.Prisma.Decimal(dto.amount);
    }
    ...
    if (dto.totalCost !== undefined) {
        updateData.totalCost = dto.totalCost
            ? new client_1.Prisma.Decimal(dto.totalCost)
            : null;
    }
    ...
    if (dto.depositAmount !== undefined) {
        updateData.depositAmount = dto.depositAmount
            ? new client_1.Prisma.Decimal(dto.depositAmount)
            : null;
    }
    if (dto.balanceAmount !== undefined) {
        updateData.balanceAmount = dto.balanceAmount
            ? new client_1.Prisma.Decimal(dto.balanceAmount)
            : null;
    }
...
exports.mapUpdateDTOToDb = mapUpdateDTOToDb;
/**
