...
const deriveSplitPaymentPlan = (input) => {
    ...
    if (!percentage || !deposit) {
        throw ApiError_1.ApiError.badRequest("Unable to determine split payment values");
    }
    if (percentage.lessThan(MIN_DEPOSIT_PERCENTAGE)) {
        throw ApiError_1.ApiError.badRequest("Deposit percentage must be at least 10%");
    }
    if (percentage.greaterThan(MAX_DEPOSIT_PERCENTAGE)) {
        throw ApiError_1.ApiError.badRequest("Deposit percentage cannot exceed 100%");
    }
    if (deposit.lessThanOrEqualTo(0)) {
        throw ApiError_1.ApiError.badRequest("Deposit amount must be greater than zero");
    }
    if (deposit.greaterThan(totalAmount)) {
        throw ApiError_1.ApiError.badRequest("Deposit amount cannot exceed booking amount");
    }
    const balance = totalAmount.minus(deposit);
    if (balance.lessThan(0)) {
        throw ApiError_1.ApiError.badRequest("Balance amount cannot be negative");
    }
    return {
...
const validateBookingPartners = async (partners) => {
    ...
    if (uniquePartnerIds.size !== partnerIds.length) {
        throw ApiError_1.ApiError.badRequest("Duplicate partners provided. Each partner can only be added once per booking.");
    }
    ...
    for (const partner of partners) {
        ...
        if (!existingPartner) {
            throw ApiError_1.ApiError.badRequest(`Partner with ID '${partner.partnerId}' does not exist. Please create the partner first or use a valid partner ID.`);
        }
    }
};
...
//# sourceMappingURL=booking.service.old.js.map