...
const createSvg = (content: string) => Buffer.from(content, "utf-8");

const randomChoice = <T,>(items: T[]): T => items[Math.floor(Math.random() * items.length)];

export const offerAssetService = {
  generateLogo: async () => {
    ...
    return offerAssetService.persistAsset("logo", createSvg(svg), "logo.svg");
  },
  ...
  generateSignature: async () => {
    ...
    return offerAssetService.persistAsset("signature", createSvg(svg), "signature.svg");
  },

  persistAsset: async (type: "logo" | "signature", buffer: Buffer, filename: string) => {
    const dir = path.resolve(config.storage.tempDir, "offers");
    ensureDir(dir);
    const uniqueName = `${type}-${Date.now()}-${randomUUID()}-${filename}`;
    const filepath = writeTempFile(dir, uniqueName, buffer);
