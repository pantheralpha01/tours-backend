...
const deriveSplitPaymentPlan = (input: SplitPaymentInput): SplitPaymentPlan => {
  ...
  if (!percentage || !deposit) {
    throw ApiError.badRequest("Unable to determine split payment values");
  }

  if (percentage.lessThan(MIN_DEPOSIT_PERCENTAGE)) {
    throw ApiError.badRequest("Deposit percentage must be at least 10%");
  }

  if (percentage.greaterThan(MAX_DEPOSIT_PERCENTAGE)) {
    throw ApiError.badRequest("Deposit percentage cannot exceed 100%");
  }
  ...
  if (deposit.greaterThan(totalAmount)) {
    throw ApiError.badRequest("Deposit amount cannot exceed booking amount");
  }

  const balance = totalAmount.minus(deposit);

  if (balance.lessThan(0)) {
    throw ApiError.badRequest("Balance amount cannot be negative");
  }
